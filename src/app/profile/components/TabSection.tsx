...
export const TabSection: React.FC<TabSectionProps> = ({ user }) => {
    ...
    const tabs: Array<{ id: TabId; label: string; icon: React.ReactNode }> = [
        ...
        { id: 'password', label: 'Password Manage', icon: <Lock className="w-5 h-5" /> }
    ];
    ...
    return (
        <div className="flex ">
            ...
            <div className="flex-1 pl-6">
                {activeTab === 'profile' && <ProfileForm user={user} onSubmit={handleSubmit} />}
                {activeTab === 'address' && (
                    <AddressSection
                        addresses={user.addresses}
                        onAddAddress={async (address) => console.log('Add address:', address)}
                        onUpdateAddress={async (index, address) => console.log('Update address:', index, address)}
                        onDeleteAddress={async (index) => console.log('Delete address:', index)}
                    />
                )}
                {activeTab === 'password' && (
                    <PasswordSection
                        onPasswordChange={async (data) => console.log('Password change:', data)}
                    />
                )}
            </div>
        </div>
